...
postSlug: "agents-md-outperforms-skills-in-our-agent-evals"
description: "AGENTS.md Outperforms Skills in Our Agent Evals"
tags:
  - skills
  - openagents
  - development
...
[ Jude GaoSoftware Engineer, Next.js ](https://twitter.com/gao_jude)
...
In one eval (the `'use cache'` directive test), "invoke first" approach wrote correct `page.tsx` but completely missed the required `next.config.ts` changes. The "explore first" approach got both.

This fragility concerned us. If small wording tweaks produce large behavioral swings, approach feels brittle for production use.
...
This wasn't what we expected. The "dumb" approach (a static markdown file) outperformed more sophisticated skill-based retrieval, even when we fine-tuned the skill triggers.
